...
sa_mlp = SeverityAssessment(_back_to_root, _data_path, _data_name, [activity_id], 'mlp_8', roc=True)
total_fpr_micro_mlp, total_tpr_micro_mlp, total_roc_auc_micro_mlp = get_roc_curve(sa_mlp)
print(get_confusion_matrices(sa_mlp))

sa_lgbm = SeverityAssessment(_back_to_root, _data_path, _data_name, [activity_id], 'lgbm', roc=True)
total_fpr_micro_lgbm, total_tpr_micro_lgbm, total_roc_auc_micro_lgbm = get_roc_curve(sa_lgbm)

sa_linear = SeverityAssessment(_back_to_root, _data_path, _data_name, [activity_id], 'logistic_l1', roc=True)
total_fpr_micro_linear, total_tpr_linear, total_roc_auc_linear = get_roc_curve(sa_linear)

sa_bayes = SeverityAssessment(_back_to_root, _data_path, _data_name, [activity_id], 'bayes', roc=True)
total_fpr_micro_bayes, total_tpr_micro_bayes, total_roc_auc_micro_bayes = get_roc_curve(sa_bayes)

sa_knn = SeverityAssessment(_back_to_root, _data_path, _data_name, [activity_id], 'knn', roc=True)
