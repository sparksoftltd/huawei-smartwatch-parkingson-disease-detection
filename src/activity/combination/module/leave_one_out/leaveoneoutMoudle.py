...
class LGBModel:
    ...
    # 训练模型
    def trainlgb_model(self, X_train, X_test, y_train, y_test, topn,weightdict):
...
def leave_subject_to_one(data, choose_people, choose_label, chooseleve, choosepeople_top,weightdict,subject_id,features,label):
    # 记录人准确率，和等级准确率
    dictlevel1 = {}
    dictlevel2 = {}
    dictlevel3 = {}
    ...
    for people, labelll in zip(choose_people, choose_label):
        ...
        test_y = test_y
        lgbtrain = LGBModel()
        models, soc, sample_label, fft = lgbtrain.trainlgb_model(train_x, test_x, train_y, test_y, choosepeople_top,weightdict)
...
def leave_subject_out(data, recodata, lebal2, trun, encoder, iencoder, chooselevel, choosepeople_top,weightdict,subject_id,features,label,dongzuo,trueall,preall):
    # 记录人准确率，和等级准确率
