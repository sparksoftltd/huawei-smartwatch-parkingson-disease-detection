    ...
    for parameter in parameter_list:
        ...
        for i in range(folder_num):
            my_model = OneVsRestClassifier(
                svm.LinearSVC(penalty='l1', dual=False, C=parameter, random_state=None, verbose=0, max_iter=3000))
...
for activity_idx in activity_idx_list:
    ...
    # # 得到0-173的列名，也就是特征名
    # temp_dataset = pd.read_csv(file_name, header=0)
    # feature_names = temp_dataset.columns.values.tolist()
    ...
    for i in range(repetition_num):
        ...
        # 选择最佳参数
        parameter_list = [0.0001, 0.001, 0.01, 0.1, 1]
        ...
        my_model = OneVsRestClassifier(
            svm.LinearSVC(penalty='l1', dual=False, C=best_parameter, random_state=None, verbose=0, max_iter=3000))
...
save_path = final_path + 'SVM_L1.csv'  # 存储路径
