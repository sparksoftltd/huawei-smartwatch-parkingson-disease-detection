...
def optimize_lgbm_params(trial, model_params_cfg):
    ...
    params = {
        ...
        "learning_rate": suggest_float(trial, model_params_cfg, "learning_rate", float_round=2),
        "max_depth": max_depth,
        ...
        "lambda_l1": suggest_float(trial, model_params_cfg, "lambda_l1", float_round=2),
        "lambda_l2": suggest_float(trial, model_params_cfg, "lambda_l2", float_round=2),
    }
...
def outer_objective(cfg: DictConfig, activity_id: List, classifier: str, combination_mode: str):
    def objective(trial):
        ...
        data_loader = PDDataLoader(
            activity_id,
            os.path.join(dataset_cfg.back_to_root, data_path, file_name),
        ...
        print("current search params: ", params)
...
def study(cfg: DictConfig, activity_id: List, classifier: str, combination_mode):
    ...
    study = optuna.create_study(direction=cfg.optuna.direction,
                                ...
                                study_name='activity_id ' + str(
                                    activity_id) + ' classifier ' + classifier + " " + combination_mode + " " + expname)

    study.set_user_attr('Completed', False)

    study.optimize(outer_objective(cfg, activity_id, classifier, combination_mode), n_trials=n_trials,
